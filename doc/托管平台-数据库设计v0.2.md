...
2\. 托管机构评论表 tb\_deposit\_comments

|**字段名**          |**类型**   |备注|
|  -------------- |---------- |-------------------------------------------------------------------------------------------------------------------------|
|  CommentID       |int        |评论序号，用于检索评论信息, auto_increment, PRI |
|  DepositID       |int32      |被评论的机构账号|
|  CommentatorID   |int32      |评论人ID, 如果是家长评论则为家长ID, 如果是公司评论，则写顾问账号，目前可以统一填写60000000|
|  CommentatorType |int32      |评论人类型，目前只支持两种：0： 家长评论， 1：公司评论|
|  Score           |float      |评论分数，0 ~ 10，精度为小数点后1位 |
|  CommentText     |varchar2   |评论的文本数据|
|  Status          |int        |评论状态，该评论是否有效，0：无效，1：有效|
|  CreateTime      |datetime   |创建时间|
|  ModifyTime      |datetime   |修改时间|
