(function() {
	"use strict";
	angular.module('ChargeListCtrl', []).controller(
			'ChargeListCtrl',
			function($scope, AuthService, NgTableParams, $state, $stateParams,
				...
				$scope.chargedetail = function(event) {

					XHDialog.chargedetail(event, function() {
						$scope.query();
						$scope.focus();
					}, function() {
						$scope.focus();
				...
				{
					$scope.filter="( Name like '%"+$scope.simpleFilter +"%' "+" or OrderID like '%"+$scope.simpleFilter +"%' "
					+" or Mobile like '%"+$scope.simpleFilter +"%' "+" or BusinessName like '%"+$scope.simpleFilter +"%' "
					 +") and ";
